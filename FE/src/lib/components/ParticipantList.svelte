<script lang="ts">
  import type { CardValue, PublicParticipant, Vote } from "@pp/shared";

  interface Props {
    participants: PublicParticipant[];
    myId: string | null;
    revealed: boolean;
    votes: Vote[];
  }

  let { participants, myId, revealed, votes }: Props = $props();

  function revealedValue(participantId: string): CardValue | null {
    const v = votes.find((vote) => vote.participantId === participantId);
    return v ? v.cardValue : null;
  }
</script>

<ul class="participants">
  {#each participants as p (p.id)}
    <li class="participant" class:me={p.id === myId}>
      <span class="name">
        {p.displayName}
        {#if p.id === myId}<span class="you">(you)</span>{/if}
        {#if p.isHost}<span class="badge host">Host</span>{/if}
      </span>

      {#if revealed}
        <span class="value" class:empty={revealedValue(p.id) === null}>
          {revealedValue(p.id) ?? "—"}
        </span>
      {:else}
        <span
          class="dot"
          class:voted={p.hasVoted}
          aria-label={p.hasVoted ? "Voted" : "Not voted yet"}
          title={p.hasVoted ? "Voted" : "Not voted yet"}
        ></span>
      {/if}
    </li>
  {/each}
</ul>

<style>
  .participants {
    list-style: none;
    margin: 0;
    padding: 0;
    display: flex;
    flex-direction: column;
    gap: var(--space-2);
  }

  .participant {
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding: var(--space-3) var(--space-4);
    border: 1px solid var(--color-border);
    border-radius: var(--radius-md);
    background: var(--color-surface);
  }

  .participant.me {
    border-color: var(--color-primary);
    background: var(--color-primary-soft);
  }

  .name {
    display: inline-flex;
    align-items: center;
    gap: var(--space-2);
    font-weight: 600;
  }

  .you {
    color: var(--color-text-muted);
    font-weight: 400;
    font-size: var(--font-size-sm);
  }

  .badge.host {
    padding: 2px 8px;
    border-radius: 999px;
    background: var(--color-primary);
    color: #fff;
    font-size: 0.7rem;
    font-weight: 700;
    text-transform: uppercase;
    letter-spacing: 0.03em;
  }

  .dot {
    width: 14px;
    height: 14px;
    border-radius: 999px;
    background: var(--color-surface-2);
    border: 1px solid var(--color-border);
  }

  .dot.voted {
    background: var(--color-success);
    border-color: var(--color-success);
  }

  .value {
    min-width: 34px;
    text-align: center;
    padding: 2px 10px;
    border-radius: var(--radius-sm);
    background: var(--color-surface-2);
    font-weight: 700;
  }

  .value.empty {
    color: var(--color-text-muted);
    font-weight: 400;
  }
</style>
